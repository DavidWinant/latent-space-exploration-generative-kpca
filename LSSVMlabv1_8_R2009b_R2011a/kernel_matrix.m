...
elseif strcmp(kernel_type,'sig_kernel')
     if nargin<4,
         omega = tanh(kernel_pars(1)*Xtrain*Xtrain' + kernel_pars(2));
     else
         omega = tanh(kernel_pars(1)*Xtrain*Xt' + kernel_pars(2));
     end
     
elseif strcmp(kernel_type,'chisquared_kernel')
...
elseif strcmp(kernel_type,'histint_kernel')
    
    if nargin<4,       
        omega=squareform(pdist(Xtrain,@histintdistance));
    else
%             omega=zeros(nb_data,size(Xt,1))';
%             for i=1:nb_data
%                 omega(:,i)=exp(-chi2distance(Xtrain(i,:),Xt)/kernel_pars(1));  
%             end;
%             omega=omega';
        omega= pdist2(Xt,Xtrain,@histintdistance);
    end
